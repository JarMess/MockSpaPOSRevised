...
public class CheckInBillPopUp extends GridPane{
    ...
    public CheckInBillPopUp(Spa mainSpa, ReceptionPane rPane){
        FXMLLoader fxmlLoader= new FXMLLoader(getClass().getResource("checkInBillPopUpFXML.fxml"));
        ...
        setHeader();
        setBillNLockers(rPane.getAvailableData());
        setTextArea();
        setButtons();
    }
    ...
    private void setBillNLockers(ObservableList<ReceptionPane.AvailableLocker> checkingIn){
        bill= new Bill();
        int totalGuests=0;
        StringBuffer sb= new StringBuffer("Lockers: ");
        for (ReceptionPane.AvailableLocker a : checkingIn) {
            sb.append(a.getLockerNumber());
            sb.append("  ");
            bill.addItem(a.getCheckInItem());
            totalGuests++;
        }
